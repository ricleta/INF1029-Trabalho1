...
int main(int argc, char *argv[]) {
  ...
  if((scalar_value == 0.0f) || (dimA_M == 0) || (dimA_N == 0) || (dimB_M == 0) || (dimB_N == 0)) {
    fprintf(stderr, "%s: erro na conversao do argumento: errno = %d\n", argv[0], errno);
...
int load_matrix(struct matrix *matrix, char *filename) {
  ...
  for(i=0; i<n; i+=8, nxt_a+=8) {
      ...
      fprintf(stderr, "Error reading from file %s: short read (less than 8 floats)\n", filename);
      return 0;
	  }
  }
